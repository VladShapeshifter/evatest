package evaautomation;


import evaautomation.model.MainPage;
...
//    @AfterMethod
    public void login(String login, String password) {
        ...
        MainPage mainPage = new MainPage(driver, wait, login, password);
        log("<br><b><i>Open main page</i></b>");
        mainPage.open();
        log("<br><b><i>Click on enter button</i></b>");
        mainPage.clickOnEnterButton();
        log("<br><b><i>Fill in email</i></b>");
        mainPage.fillInEmail();
    ...
    public void login(String login, String password, int accountsQty) {
        // TODO implement logging in to Admin Panel
        MainPage mainPage = new MainPage(driver, wait, login, password);
        log("<br><b><i>Open main page</i></b>");
        if (!TabSelector.isItLastAccount()) {
            TabSelector.setAccountsQty(accountsQty);
            mainPage.open();
            new Actions(driver).sendKeys(Keys.CONTROL).sendKeys("t").build().perform();
            ArrayList<String> tabs = new ArrayList<String>(driver.getWindowHandles());
            int i  = TabSelector.getCurrentTab();
            driver.switchTo().window(tabs.get(i++));
            TabSelector.setCurrentTab(i);
        } else {
            mainPage.open();
        }
